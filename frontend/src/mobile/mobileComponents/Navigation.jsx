...
import { RiFolderSettingsFill, RiFolderSettingsLine, RiFolderSharedFill, RiFolderSharedLine } from "react-icons/ri";
...
export default function Navigation({ menuOpen, setMenuOpen }) {
  ...
  return (
    <ul className={`bg-background h-full fixed transition-transform sm:top-0 sm:mt-[64px] sm:right-0 max-sm:bottom-0 max-sm:h-auto max-sm:w-full max-sm:flex max-sm:flex-row max-sm:justify-between ${menuOpen ? 'w-[240px]' : 'w-[96px]'}`}>
        ...
        <li className={`p-3 w-full ${isActive ? 'hover:bg-slate-200' : 'hover:bg-slate-100' }  flex items-center cursor-pointer ${menuOpen ? 'flex-row gap-5 pl-6 text-sm' : 'flex-col gap-2 text-xs'}`}>
            {!isActive ? <RiFolderSettingsFill size={20}/> : <RiFolderSettingsLine size={20}/> } Status
        </li>
     </ul>
  )
}
